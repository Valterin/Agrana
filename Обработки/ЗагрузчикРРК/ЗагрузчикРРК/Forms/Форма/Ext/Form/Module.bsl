...
Процедура ЗаполнитьФактСогласноНорме(Показатель, ТЧФакта, ТЧНормы)
	...
	Для Каждого НайденнаяСтрока Из МассивСтрок Цикл							
		...
		УстановитьДатуРедактированияСтроки(НайденнаяСтрока);
	    //ОбновитьСтрокуФактаПоСтрокеДереваНаСервере(Элементы.ДеревоНормативов.ТекущаяСтрока);
		//Для Каждого Стр Из ДеревоНормативов.ПолучитьЭлементы() Цикл
		//Элементы.ДеревоНормативов.Развернуть(Стр.ПолучитьИдентификатор());
		//КонецЦикла;
...
&НаСервере
Процедура УстановитьДатуРедактированияСтроки(ТекСтрока)
	
	ТекСтрока.ФактическаяДатаПолученияРезультата = ДатаРедактированияСтроки();
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДатаРедактированияСтроки()
