...
Функция СформироватьПечатнуюФорму(СсылкаНаДокумент, ОбъектыПечати)Экспорт
	...
	КонецЕсли;
	Если СсылкаНаДокумент.ОбеспечениеМатериаламиИРаботами.Количество() > 0 Тогда
		ОтборВода = Новый Структура("Номенклатура" , ВодаДляПоиска);
		ТЗМатериалы =  СсылкаНаДокумент.ОбеспечениеМатериаламиИРаботами.Выгрузить(ОтборВода,"КоличествоУпаковок");
		ТЗМатериалы.Свернуть(,"КоличествоУпаковок");  
		Если ТЗМатериалы.Количество() > 0 Тогда
			ОбластьТело.Параметры.Вода = Окр(ТЗМатериалы[0].КоличествоУпаковок,2); 	
		КонецЕсли;
	КонецЕсли;
	...
	ВходныеДанные =  ПолучитьСсылкуДляХарактеристики(СсылкаНаДокумент.Распоряжение);
	Если  НЕ ВходныеДанные = Неопределено Тогда
	...
	Если СсылкаНаДокумент.ОбеспечениеМатериаламиИРаботами.Количество() > 0 Тогда
		...
		ТЗМатериалыИУслуги =  СсылкаНаДокумент.ОбеспечениеМатериаламиИРаботами.Выгрузить();
		Для Каждого Строка ИЗ ТЗМатериалыИУслуги Цикл 
			Если НЕ Строка.Номенклатура.ВидНоменклатуры = ВидНоменклатуры  Тогда
				...
				ТаблицыМатериалыИРаботы.Параметры.Ключ       = Строка.Операция;
...
Функция ПолучитьСсылкуДляХарактеристики(Распоряжение)   
